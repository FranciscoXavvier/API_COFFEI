import { getConnection, productoQuerys, sql } from "../database";
...
export const buscarProductosConPuntos = async (req, res, next) => {
  ...
  try {
    ...
    result = await pool.request().query(productoQuerys.ListarTodosConPuntos);

    if (result.recordset) {
      if(result.recordset.length < 1){
        return res.status(404).json({ mensage: "No se encontraron Productos con puntos" });
      
      }else{
        res.status(200).json(result.recordset);    
      }
    } else {
      return res.status(500).json({ mensage: "Error al obtener los productos" });
    }
